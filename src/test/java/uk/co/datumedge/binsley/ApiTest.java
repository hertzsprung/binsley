...
public class ApiTest {
    ...
    void test() throws IOException, InterruptedException {
        try (CognitoIdentityProviderClient cognito = CognitoIdentityProviderClient.create();
            ...
            HttpResponse<InputStream> oauth2Response = HttpClient.newHttpClient()
                    .send(oauth2Request, HttpResponse.BodyHandlers.ofInputStream());
            ...
            HttpResponse<String> response = HttpClient.newHttpClient()
                    .send(HttpRequest.newBuilder().GET().header("Authorization", accessToken).uri(apiBaseUri).build(), HttpResponse.BodyHandlers.ofString());
